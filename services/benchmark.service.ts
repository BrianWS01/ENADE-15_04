...
export async function processBenchmarkImport(rows: any[]): Promise<{ success: number, error: number, coursesCreated: number }> {
  ...
  for (const row of rows) {
    try {
      ...
      // Os KPIs reais (enadeScore etc.) serão preenchidos quando alunos forem importados.
